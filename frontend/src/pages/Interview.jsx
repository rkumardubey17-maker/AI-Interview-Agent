...
export default function InterviewPage() {
   ...
   const [progress, setProgress] = useState({ questionNumber: 1, totalQuestions: 8 });
   ...
   const startSession = async () => {
      setIsLoading(true);
      setError(null);
      console.log("CANDIDATE SENT TO BACKEND:", candidate);
      try {
          console.log("MISSIONS:", candidate?.missions);
         const response = await axios.post(`${BACKEND_URL}/api/interview`, { candidate });

         const data = response.data.data;
         console.log(data);

         setSessionId(data.sessionId);
         setProgress({ questionNumber: 1, totalQuestions: 8 });
   ...
   const handleSendMessage = async (e) => {
      ...
      try {
         const response = await axios.post( `${BACKEND_URL}/api/interview`, { sessionId, message: userText } );
         ...
         // Interview continues
         setProgress((prev) => ({ ...prev, questionNumber: prev.questionNumber + 1 }));
         setMessages((prev) => [ ...prev, { sender: 'interviewer', text: data.reply } ]);
